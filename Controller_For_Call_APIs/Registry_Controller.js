...
registry_router.get("/registry/:registryId", async (req, res) => {
  try {
    const registryItem = await Registry.findOne({
      registry_id: req.params.registryId,
    });
    if (!registryItem) {
...
registry_router.put("/registry/:registryId", async (req, res) => {
  try {
    const updatedRegistryItem = await Registry.findOneAndUpdate(
      { registry_id: req.params.registryId },
      req.body,
      { new: true }
    );
...
registry_router.delete("/registry/:registryId", async (req, res) => {
  try {
    const deletedRegistryItem = await Registry.findOneAndDelete({
      registry_id: req.params.registryId,
    });
