...
budget_router.post('/budgets', async (req, res) => {
  ...
  budget_router.put('/budgets/:budgetId', async (req, res) => {
    try {
      const updatedBudget = await Budget.findOneAndUpdate({ budget_id: req.params.budgetId }, req.body, { new: true });
      if (!updatedBudget) {
        res.status(404).json({ error: 'Budget not found' });
      } else {
  ...
  budget_router.delete('/budgets/:budgetId', async (req, res) => {
    try {
      const deletedBudget = await Budget.findOneAndDelete({ budget_id: req.params.budgetId });
      if (!deletedBudget) {
        res.status(404).json({ error: 'Budget not found' });
      } else {
