...
vendor_router.post('/vendors', async (req, res) => {
  ...
  vendor_router.put('/vendors/:vendorId', async (req, res) => {
    try {
      const updatedVendor = await Vendor.findOneAndUpdate({ vendor_id: req.params.vendorId }, req.body, { new: true });
      if (!updatedVendor) {
        res.status(404).json({ error: 'Vendor not found' });
      } else {
  ...
  vendor_router.delete('/vendors/:vendorId', async (req, res) => {
    try {
      const deletedVendor = await Vendor.findOneAndDelete({ vendor_id: req.params.vendorId });
      if (!deletedVendor) {
        res.status(404).json({ error: 'Vendor not found' });
      } else {
