...
auth_route.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      const isVisited = await ecommarse.site_visited_user.push(email).save();
      if (!isVisited) {
        return res.status(400).json({
          success: false,
          msg: `User with email ${email} already exist`,
...
auth_route.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (isUserExist && (await bcrypt.compare(password, isUserExist.password))) {
      ...
      //   httpOnly: true,
      //   expires: new Date(Date.now() + 60 * 11),
      // });
      return res
        .status(200)
...
auth_route.post("/forgot_password", async (req, res) => {
  ...
  try {
    ...
    const message = {
      ...
      html: `<p>Hii, ${isUserExist.name} </p></br> <h5>Your password is <b>${userPassword}</b></h5></br><p>Please do not share this password to anyone.</br></br></br></br></br>no reply</p>`,
    };
